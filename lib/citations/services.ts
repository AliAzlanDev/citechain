  ...
  try {
    if (direction === 'backward' || direction === 'both') {
      const backwardResults =
        await searchOpenAlexBackwardCitations(openAlexInputs)
      backward.push(...backwardResults)
    }

    if (direction === 'forward' || direction === 'both') {
      const forwardResults =
        await searchOpenAlexForwardCitations(openAlexInputs)
...
async function searchOpenAlexBackwardCitations(
  ...
  for (const batch of batches) {
    ...
    const paperResponse: OpenAlexCitationResponse =
      await rateLimitedFetch(paperUrl)
    ...
    if (allReferencedWorks.size > 0) {
      ...
      const batchCitations =
        await fetchOpenAlexWorkDetails(referencedWorksArray)
...
export async function searchSemanticScholarCitations(
  ...
  try {
    ...
    for (const batch of batches) {
      ...
      const papers: (SemanticScholarPaper | null)[] =
        await fetchFromSemanticScholar<(SemanticScholarPaper | null)[]>(
          s2Identifiers,
          [...CITATION_CONFIG.SEMANTIC_SCHOLAR_FIELDS],
        )
...
function transformS2CitationsToCitations(
  s2Citations: (SemanticScholarCitation | null)[],
  abstractsMap: Map<string, string>,
): Citation[] {
  return s2Citations
    .filter((citation): citation is SemanticScholarCitation =>
