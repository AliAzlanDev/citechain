...
export function generateRIS(references: Citation[]): string {
  ...
  for (const ref of references) {
    ...
    // Abstract source
    if (ref.abstract_source) {
      risContent += `DS  - ${formatProviderName(ref.abstract_source)}\n`
    }

    // Database provider
    if (ref.database_provider) {
      risContent += `DP  - ${formatProviderName(ref.database_provider)}\n`
    }
...
function formatProviderName(provider: 'openalex' | 'semantic_scholar'): string {
  return provider === 'semantic_scholar' ? 'semantic scholar' : 'openalex'
}
